...
class cwpds__ET_TEXT_DIVIDER_MODULE extends ET_Builder_Module {
	function init() {
		$this->name       = esc_html__( 'Text Divider', 'cwp-ds' );
		$this->plural     = esc_html__( 'Text Dividers', 'cwp-ds' );
		$this->slug       = 'et_pb_text_divider';
		$this->vb_support = 'on';
		...
		$this->advanced_fields = array(
			...
			'fonts'                 => array(
				'header_text' => array(
					'label'    => esc_html__( 'Title', 'cwp-ds' ),
					'css'      => array(
						'main' => " %%order_class%% .text_divider_wrapper .text_element  h1,
						%%order_class%% .text_divider_wrapper .text_element  h2,
						%%order_class%% .text_divider_wrapper .text_element  h3,
						%%order_class%% .text_divider_wrapper .text_element  h4,
						%%order_class%% .text_divider_wrapper .text_element  h5,
						%%order_class%% .text_divider_wrapper .text_element  h6      
						"),
					'header_level' => array(
						'default' => 'h4',
					),
	...
	function get_fields() {
		$fields = array(
		    ...
		    'cwp_use_text' => array(
				'label'           => esc_html__( 'Use Text', 'cwp-ds' ),
				'type'            => 'yes_no_button',
			...
			'cwp_text_align'        => array(
			    'label'       => esc_html__( 'Text Alignment', 'cwp-ds' ),
			 ...
			 'cwp_text_bg_color' => array(
				'label'             => esc_html__( 'Bg Color', 'cwp-ds' ),
				'type'              => 'color-alpha',
				'description'       => esc_html__( '', 'cwp-ds' ),
				'default'           => '',
				'tab_slug'          => 'advanced',
			...
			'cwp_text_d_url_new_window' => array(
	...
	function render( $attrs, $content = null, $render_slug ) {
		$cwp_use_text                    = $this->props['cwp_use_text'];
		$cwp_text_spacing                = $this->props['cwp_text_spacing'];
		$cwp_d_color                     = $this->props['cwp_d_color'];
		$cwp_d_weight                    = $this->props['cwp_d_weight'];
		$cwp_text_align                  = $this->props['cwp_text_align'];
		...
		$divider_title                   = $this->props['divider_title'];
		$header_text_level               = $this->props['header_text_level'];
		$cwp_text_bg_color               = $this->props['cwp_text_bg_color'];
		$cwp_text_d_url                  = $this->props['cwp_text_d_url'];
		$cwp_text_d_url_new_window       = $this->props['cwp_text_d_url_new_window'];
		$cwp_text_padding  	                    = $this->props['cwp_text_padding'];
		$cwp_text_padding_tablet  	            = $this->props['cwp_text_padding_tablet'];
		$cwp_text_padding_phone  	            = $this->props['cwp_text_padding_phone'];
		$cwp_text_padding_last_edited            = $this->props['cwp_text_padding_last_edited'];
		$cwp_text_padding_responsive_active      = et_pb_get_responsive_status($cwp_text_padding_last_edited);
		$cwp_d_size  	                         = $this->props['cwp_d_size'];
		...
		ET_Builder_Element::set_style( $render_slug, array(
			'selector'    => '%%order_class%% .text_divider_wrapper .text_element',
			'declaration' => sprintf('padding:%1$s;',$cwp_text_padding),
		));
		if ( $cwp_text_padding_responsive_active ) {
			ET_Builder_Element::set_style( $render_slug, array(
				'selector'    => '%%order_class%% .text_divider_wrapper .text_element',
				'declaration' => sprintf('padding:%1$s;', $cwp_text_padding_tablet),
		...
		if ($cwp_text_padding_responsive_active) {
			ET_Builder_Element::set_style( $render_slug, array(
				'selector'    => '%%order_class%% .text_divider_wrapper .text_element',
				'declaration' => sprintf('padding:%1$s;', $cwp_text_padding_phone),
	...
	// Fancy Style Work
	 $svg_path = '';
			...
			if (  'fancy' === $cwp_select_style  && ('off' === $cwp_use_text && $cwp_style_fancy) ) {
				ET_Builder_Element::set_style( $render_slug, array(
					'selector'    => '%%order_class%% .text_divider_wrapper .seprator_element::before',
			...
			if ( 'on' === $cwp_use_text && ( 'fancy' === $cwp_select_style  && ( $cwp_style_fancy && 'left' === $cwp_text_align ) ) ) {
				ET_Builder_Element::set_style( $render_slug, array(
					'selector'    => '%%order_class%% .text_divider_wrapper .seprator_element::after',
					'declaration' =>  sprintf('background: url("%1$s");', $svg_path),
				));
				ET_Builder_Element::set_style( $render_slug, array(
					'selector'    => '%%order_class%% .text_divider_wrapper .seprator_element::after',
			...
			if ( 'on' === $cwp_use_text && (  'fancy' === $cwp_select_style  &&   ($cwp_style_fancy && 'center' === $cwp_text_align) ) ) {
				ET_Builder_Element::set_style( $render_slug, array(
					'selector'    => '%%order_class%% .text_divider_wrapper .seprator_element::before, %%order_class%% .text_divider_wrapper .seprator_element::after',
					'declaration' =>  sprintf('background: url("%1$s");', $svg_path),
				));
				ET_Builder_Element::set_style( $render_slug, array(
					'selector'    => '%%order_class%% .text_divider_wrapper .seprator_element::before, %%order_class%% .text_divider_wrapper .seprator_element::after',
			...
			if ( 'on' === $cwp_use_text && (  'fancy' === $cwp_select_style  &&  ( $cwp_style_fancy && 'right' === $cwp_text_align ) ) ) {
				ET_Builder_Element::set_style( $render_slug, array(
					'selector'    => '%%order_class%% .text_divider_wrapper .seprator_element::before',
					'declaration' =>  sprintf('background: url("%1$s");', $svg_path),
				));
		...
		if ($cwp_text_spacing) {
			ET_Builder_Element::set_style( $render_slug, array(
				'selector'    => '%%order_class%% .text_divider_wrapper .text_element',
				'declaration' =>  sprintf('margin-left: %1$s;', $cwp_text_spacing),
			));
			ET_Builder_Element::set_style( $render_slug, array(
				'selector'    => '%%order_class%% .text_divider_wrapper .text_element',
				'declaration' =>  sprintf('margin-right: %1$s;', $cwp_text_spacing),
			));
		}
		
		if ($cwp_text_bg_color) {
			ET_Builder_Element::set_style( $render_slug, array(
				'selector'    => '%%order_class%% .text_divider_wrapper .text_element',
				'declaration' =>  sprintf('background: %1$s;', $cwp_text_bg_color),
			));	
		}

		// Divider CSS styliing

		if(  'simple' === $cwp_select_style && 'left' === $cwp_text_align) {
			if ($cwp_style_simple) {
				ET_Builder_Element::set_style( $render_slug, array(
					'selector'    => '%%order_class%% .text_divider_wrapper .seprator_element::after',
		...
		if( 'simple' === $cwp_select_style && 'center' === $cwp_text_align) {
			if ($cwp_style_simple) {
				ET_Builder_Element::set_style( $render_slug, array(
					'selector'    => '%%order_class%% .text_divider_wrapper .seprator_element::before,%%order_class%% .text_divider_wrapper .seprator_element::after',
					'declaration' =>  sprintf('border-top-style: %1$s;', $cwp_style_simple),
		...
		if( 'simple' === $cwp_select_style && 'right' === $cwp_text_align) {
			if ($cwp_style_simple) {
				ET_Builder_Element::set_style( $render_slug, array(
					'selector'    => '%%order_class%% .text_divider_wrapper .seprator_element::before',
					'declaration' =>  sprintf('border-top-style: %1$s;', $cwp_style_simple),
		...
		ET_Builder_Element::set_style( $render_slug, array(
			'selector'     => '%%order_class%% .text_divider_wrapper .seprator_element',
		 ...
		 $output = sprintf(
			'%2$s
			 ...
			 %3$s',
			'on' === $cwp_use_text ? sprintf('<div class="text_element"><%1$s class="et_pb_module_header">%2$s</%1$s></div>', et_pb_process_header_level( $header_text_level, 'h4' ),$divider_title) : '',
			'' !== $cwp_text_d_url ? sprintf('<a href="%1$s" %2$s>', $cwp_text_d_url, 'on' === $cwp_text_d_url_new_window ? 'target="_blank" ' : '') : '',
			'' !== $cwp_text_d_url ? '</a>' : ''	
        );
...
new cwpds__ET_TEXT_DIVIDER_MODULE;
